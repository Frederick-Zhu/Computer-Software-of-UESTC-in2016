...
void addbysort(LinkList l, ElemType data);
...
int main(int argc, char *argv[]) {
    ...
    printlist(A);
    printf("\n\n");
    Student s = newStudent(rand()%(101), rand()%(101));
    printf("insert: number:  %d    math:%d\n", s.number, s.math);
//    linklistsort(A);
    addbysort(A, s);
//    printf("insert: number:  %d    math:%d\n", s.number, s.math);
    printlist(A);
...
void addbysort(LinkList l, ElemType data) {
    ...
    if(p) {    //has head node
            if(p->next) {    //has elem
                p = p->next;
                while(p && p->data.number<data.number) {
                    p = p->next;
                }
                if(p) {
                    insert(l, findbynumber(l, p->data.number), data);
                }
                else {
                    add(l, data);
                }
